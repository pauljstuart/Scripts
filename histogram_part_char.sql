col PARAM1 new_value 1 format A20
col PARAM2 new_value 2 format A20
col PARAM3 new_value 3 format A20
col PARAM4 new_value 4 format A20

select null PARAM1, null PARAM2, null PARAM3, null PARAM4 from dual where 1=2;
select nvl( '&1','&_USER') PARAM1, nvl('&2','%') PARAM2, nvl('&3','%')  PARAM3, nvl('&4','%')  PARAM4 from dual ;
...
prompt
prompt Histograms : owner &OWNER, table &TABLE_NAME, partition &PART_NAME,  column &COLUMN_NAME , &DAYS_AGO days ago  
prompt

column end_point format A10
column endpoint_value format 9999999999999999999999999999999999999999
column bucket_number format 999
...
inner join dba_objects DO on DO.owner = PH.owner and DO.object_name = PH.table_name AND DO.subobject_name = PH.partition_name
  ...
  and PH.partition_name like '&PART_NAME'
-- AND BUCKET_NUMBER = 1
