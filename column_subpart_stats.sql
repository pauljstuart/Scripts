...
prompt
prompt subpartition column statistics, owner &OWNER, table &TABLE_NAME, partition &SUBPART_NAME,  column &COLUMN_NAME , &DAYS_AGO days ago  
prompt

SELECT
  DT.OWNER ,
  DT.TABLE_NAME ,
  DT.subPARTITION_NAME,
  DT.COLUMN_NAME,
  DT.LAST_ANALYZED ,
  DT.SAMPLE_SIZE ,
  DT.NUM_DISTINCT ,
  ...
  CREATED 
from 
      dba_subpart_col_statistics DT
inner join dba_objects DO on DO.owner = DT.owner and DO.object_name = DT.table_name AND DO.subobject_name = DT.SUBpartition_name
...
AND DT.SUBpartition_name like '&SUBPART_NAME'
